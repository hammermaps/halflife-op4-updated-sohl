...
- **Implemented and playtest-verified (2026-09-08):**
  ...
  - [`docs/designs/hybrid-ai-core-phase-c.md`](docs/designs/hybrid-ai-core-phase-c.md) — squad-shared enemy memory only (confidence floored at 0.7 when squad-informed but not directly seen, per spec §8-9). Squad blackboard/roles were investigated and found **already implemented** by existing `CSquadMonster` mechanics (`m_vecEnemyLKP` sharing, `m_flLastEnemySightTime`, squad slots) — not rebuilt. Confirmed via a 16k-line activity-log analysis on `c2a5f`: squad-shared floor fires correctly, `ATTACK` never fires without direct visibility. Two bugs found and fixed along the way: a confidence-decay double-counting bug latent since Phase B, and `ScoreCover` scoring an unknown enemy above zero (harmless in-game, misleading in logs).
  - **Local test convenience:** the deployed test mod's `spserver.cfg` (not in this repo — local install only) auto-sets `ai_hybrid`/`ai_hybrid_debug`/`ai_hybrid_log` to 1 on every singleplayer launch. The compiled-in CVar defaults in `dlls/game.cpp` stay 0.
- **Not started:** squad roles for FLANK/SUPPRESS (deferred until those actions exist to assign roles to), and further score-tuning beyond what the `c2a5f` log analysis already validated.
