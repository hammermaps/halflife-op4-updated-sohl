...
- **Implemented, manual playtest still open:** [`docs/designs/hybrid-ai-core-phase-c.md`](docs/designs/hybrid-ai-core-phase-c.md) — squad-shared enemy memory only (confidence floored at 0.7 when squad-informed but not directly seen, per spec §8-9). Squad blackboard/roles were investigated and found **already implemented** by existing `CSquadMonster` mechanics (`m_vecEnemyLKP` sharing, `m_flLastEnemySightTime`, squad slots) — not rebuilt. Also fixed a confidence-decay double-counting bug that was latent since Phase B. Read this doc's Open Questions before the next session — the actual in-game squad-sharing behavior hasn't been playtested yet.
- **Not started:** squad roles for FLANK/SUPPRESS (deferred until those actions exist to assign roles to), and the score-tuning noted as first-pass in the Phase B doc.
