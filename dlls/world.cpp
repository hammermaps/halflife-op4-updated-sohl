...
bool CDecal::KeyValue(KeyValueData* pkvd)
{
	if (FStrEq(pkvd->szKeyName, "texture"))
	{
		...
		if (pev->skin < 0)
		{
			LOG_INFO("Can't find decal %s", pkvd->szValue);
		}
...
void CGlobalState::DumpGlobals()
{
	...
	while (pTest)
	{
		LOG_INFO("%s: %s (%s)", pTest->name, pTest->levelName, estates[pTest->state]);
		pTest = pTest->pNext;
	}
}
//#endif
...
void CWorld::Precache()
{
	...
	if (!pSoundEnt)
	{
		LOG_INFO("**COULD NOT CREATE SOUNDENT**");
	}
	...
	{ // Load the node graph for this level
		...
		{ // couldn't load, so alloc and prepare to build a graph.
			LOG_INFO("*Error opening .NOD file");
			WorldGraph.AllocNodes();
		}
		else
		{
	...
	{
		LOG_DEBUG("Chapter title: %s", STRING(pev->netname));
